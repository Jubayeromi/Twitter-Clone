...
const EshaGupta = () => {
   const [post, setPost] = useState([]);
      ...
      const ESHA_BOARD_URL = ['https://www.pinterest.com/search/pins/?q=esha%20gupta%20hot&rs=typed','https://www.pinterest.com/search/pins/?q=esha%20gupta%20in%20high%20heels%20hot%20pics&rs=typed','https://in.pinterest.com/search/pins/?q=esha%20gupta%20bold%20pics&rs=typed','https://www.pinterest.com/pin/1147362442597726971/']; 
      ...
      useEffect(() => {
        ...
        const apiUrl = `https://twitter-clone-wwk7.onrender.com/api/board/all?url=${encodeURIComponent(ESHA_BOARD_URL[index])}`;
    
        axios
          .get(apiUrl)
          .then((res) => setPost(res.data.posts || []))
          .catch((err) => console.error('Error fetching Esha images:', err))
          .finally(() => setLoading(false));
      }, [index]);
  return (
    <div>
       <div className="max-w-2xl mx-auto py-4">
      <h1 className="text-2xl font-bold text-center mb-6">Esha Gupta Pins</h1>

      {loading && (
        <div className="text-center py-10 text-gray-400 font-semibold">
          Fetching Esha images from Pinterest...
        </div>
      )}
      ...
      <div className='flex gap-5 items-center w-full h-20 justify-center'>
        ...
        <div><button onClick={() => {
          if (index < ESHA_BOARD_URL.length - 1) {
            setIndex(index + 1)
            window.scrollTo({
              top: 0,
              behavior: 'smooth'
