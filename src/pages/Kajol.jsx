...
const Kajol = () => {
     ...
     const [post, setPost] = useState([]);
      ...
      const KAJOL_BOARD_URL = ['https://www.pinterest.com/search/pins/?q=kajol%20devgan%20hot&rs=typed','https://in.pinterest.com/search/pins/?q=kajol%20devgan%20in%20high%20heels%20bold%20pics&rs=typed']; 
      ...
      useEffect(() => {
        ...
        const apiUrl = `https://twitter-clone-wwk7.onrender.com/api/board/all?url=${encodeURIComponent(KAJOL_BOARD_URL[index])}`;
    
        axios
          .get(apiUrl)
          .then((res) => setPost(res.data.posts || []))
          .catch((err) => console.error('Error fetching kajol images:', err))
          .finally(() => setLoading(false));
      }, [index]);
  return (
    <div>
      <div className="max-w-2xl mx-auto py-4">
        <h1 className="text-2xl font-bold text-center mb-6">Kajol Pins</h1>

        {loading && (
          <div className="text-center py-10 text-gray-400 font-semibold">
            Fetching kajol images from Pinterest...
          </div>
        )}
        ...
        <div><button onClick={() => {
          if (index < KAJOL_BOARD_URL.length - 1) {
            setIndex(index + 1)
            window.scrollTo({
              top: 0,
              behavior: 'smooth'
