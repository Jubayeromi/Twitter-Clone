...
const Kim = () => {
     ...
     const [post, setPost] = useState([]);
      ...
      const KIM_BOARD_URL = ['https://www.pinterest.com/search/pins/?q=kim%20kardashian%20hot&rs=typed','https://www.pinterest.com/search/pins/?q=kim%20kardashian%20in%20high%20heels%20hot%20pics&rs=typed']; 
      ...
      useEffect(() => {
        ...
        const apiUrl = `https://twitter-clone-wwk7.onrender.com/api/board/all?url=${encodeURIComponent(KIM_BOARD_URL[index])}`;
    
        axios
          .get(apiUrl)
          .then((res) => setPost(res.data.posts || []))
          .catch((err) => console.error('Error fetching KIM images:', err))
          .finally(() => setLoading(false));
      }, [index]);
  return (
    <div>
      <div className="max-w-2xl mx-auto py-4">
        <h1 className="text-2xl font-bold text-center mb-6">Kim Pins</h1>

        {loading && (
          <div className="text-center py-10 text-gray-400 font-semibold">
            Fetching KIm images from Pinterest...
          </div>
        )}
        ...
        <div><button onClick={() => {
          if (index < KIM_BOARD_URL.length - 1) {
            setIndex(index + 1)
            window.scrollTo({
              top: 0,
              behavior: 'smooth'
